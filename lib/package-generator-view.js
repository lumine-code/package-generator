...
class PackageGeneratorView {
  ...
  async confirm() {
    ...
    this.inputDialogView.update({ errorMessage: null, loadingMessage: "Generating package…" });
    try {
      ...
      this.inputDialogView.update({ errorMessage: error.message });
    } finally {
  ...
  validPackagePath(packagePath = this.getPackagePath()) {
    if (!path.basename(packagePath)) {
      this.inputDialogView.update({ errorMessage: "Enter a package name." });
      return false;
    }
    if (fs.existsSync(packagePath)) {
      this.inputDialogView.update({ errorMessage: `Path already exists: ${packagePath}` });
      return false;
    }
    return true;
  }
